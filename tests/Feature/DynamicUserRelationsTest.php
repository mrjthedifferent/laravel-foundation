...
/**
 * src/Models/User.php no longer defines devices()/firebaseTokens()/documents()/
 * loginHistory()/latestLogin() directly — each module registers its own via
 * User::resolveRelationUsing() in its own ServiceProvider::boot(), so the base
 * model does not import module classes. This proves the dynamic registration
 * actually resolves, the same way the removed methods did.
 */
