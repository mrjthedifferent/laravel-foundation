...
(cd "$B" && npm install --no-audit --no-fund --ignore-scripts \
    ...
    phosphor-icons@1.4.2 @fortawesome/fontawesome-free@5.15.4 inter-ui@4)

rm -rf "$A/vendor" "$A/icons" "$A/fonts"
mkdir -p "$A/css" "$A/js" "$A"/vendor/{select2,sweetalert2,ace,quill} \
    "$A/icons/phosphor/fonts" "$A"/icons/fontawesome/{css,webfonts} "$A/fonts/inter"
...
cp "$N"/@fortawesome/fontawesome-free/css/all.min.css "$A/icons/fontawesome/css/"
cp "$N"/@fortawesome/fontawesome-free/webfonts/*.woff2 "$N"/@fortawesome/fontawesome-free/webfonts/*.woff "$A/icons/fontawesome/webfonts/"
