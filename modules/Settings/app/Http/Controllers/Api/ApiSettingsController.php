...
class ApiSettingsController extends Controller
{
    ...
    public function appSettings(Request $request): JsonResponse
    {
        ...
        // Prefer the per-platform key; fall back to any legacy single key still
        // stored (pre-migration) so existing deployments keep working.
        $platformKey = $settings["google_maps_api_key_{$platform}"] ?? '';
        $settings['google_maps_api_key'] = $platformKey !== ''
            ? $platformKey
