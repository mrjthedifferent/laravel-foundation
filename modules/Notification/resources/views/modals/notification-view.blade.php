...
<div class="modal-body">
    <div class="row g-3 mb-3">
        <div class="col-md-6">
            <h6 class="fw-bold fs-xs text-muted text-uppercase mb-1">{{ __('notification::notification.modal.type_label') }}</h6>
            @php
        ...
        <div class="col-md-6">
            <h6 class="fw-bold fs-xs text-muted text-uppercase mb-1">{{ __('foundation::foundation.common.status') }}</h6>
            @if ($notification->read_at)
                <span class="badge bg-success-subtle text-success border border-success-subtle">{{ __('notification::notification.modal.status_read') }}</span>
            @else
                <span class="badge bg-warning-subtle text-warning border border-warning-subtle">{{ __('notification::notification.modal.status_unread') }}</span>
            @endif
        </div>
    </div>

    <div class="mb-3">
        <h6 class="fw-bold fs-xs text-muted text-uppercase mb-1">{{ __('notification::notification.modal.date_label') }}</h6>
    ...
    <div class="mb-3">
        <h6 class="fw-bold fs-xs text-muted text-uppercase mb-1">{{ __('notification::notification.modal.message_label') }}</h6>
        ...
        @if($notification->download_url)
            <a href="{{ $notification->download_url }}" class="btn btn-success btn-sm" target="_blank" rel="noopener">
                <i class="ph-download-simple me-1"></i>{{ __('notification::notification.modal.download_file') }}
            </a>
        @endif
    </div>
    ...
    @if(count($notification->data_payload) > 0)
        <details>
            <summary class="fw-bold fs-xs text-muted text-uppercase cursor-pointer">{{ __('notification::notification.modal.additional_data') }}</summary>
...
<div class="modal-footer">
    <div class="d-flex w-100 justify-content-between">
            ...
            @if ($notification->read_at)
                <form action="{{ route('admin.notification.mark-as-unread', $notification->id) }}" method="POST"
                    ...
                    <button type="submit" class="btn btn-warning btn-sm">
                        <i class="ph-envelope-simple me-1"></i> {{ __('notification::notification.modal.mark_as_unread') }}
                    </button>
                </form>
            @else
                <form action="{{ route('admin.notification.mark-as-read', $notification->id) }}" method="POST"
                    ...
                    <button type="submit" class="btn btn-success btn-sm">
                        <i class="ph-envelope-open me-1"></i> {{ __('notification::notification.modal.mark_as_read') }}
                    </button>
                </form>
            @endif
        </div>
        <div>
            ...
            <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">{{ __('foundation::foundation.common.close') }}</button>
        </div>
    </div>
</div>
